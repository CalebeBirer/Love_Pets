...
def listar_agendamentos(request):
    if request.method == 'POST':
        ...
        if agendamento_id and action:
            try:
                agendamento = get_object_or_404(Agendamento, id=agendamento_id)

                if action == 'finalizar' and request.user.cargo == 'W':
                    agendamento.finalizado = True
                    agendamento.save()
                    messages.success(request, 'Agendamento finalizado com sucesso!')

                elif action == 'cancelar' and request.user.cargo == 'W':
                    agendamento.delete()
                ...
                messages.error(request, f'Erro ao atualizar agendamento: {e}')
